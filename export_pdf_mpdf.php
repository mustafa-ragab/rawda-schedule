...
// محاولة استخدام mPDF إذا كان متاحاً
$useMPDF = false;
...
<body>
    ...
    <table>
        <thead>
            <tr>
                <th style="background: #e8e8e8;">المكتب</th>';
        
        $headerStartDate = new DateTime($week['start_date']);
        $dayOfWeek = (int)$headerStartDate->format('w');
        $dayOfWeek = ($dayOfWeek == 0) ? 1 : ($dayOfWeek == 6 ? 0 : $dayOfWeek + 1);
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            $html .= '<th>' . htmlspecialchars($dayName) . '<br><span style="font-size: 11px; color: #666;">' . $dayNum . '-' . $monthNum . '</span></th>';
        }
        ...
        $dayOfWeek = (int)$displayStartDate->format('w');
        $dayOfWeek = ($dayOfWeek == 0) ? 1 : ($dayOfWeek == 6 ? 0 : $dayOfWeek + 1);
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            if ($hasMenFile) {
                ...
                // رابط قابل للنقر في PDF - استخدام href مباشر مع target="_blank"
                $html .= '<a href="' . htmlspecialchars($fullMenUrl, ENT_QUOTES, 'UTF-8') . '" target="_blank" class="button button-men" style="text-decoration: none !important; color: #FFFFFF !important; display: inline-block;">ر</a>';
            } else {
                $html .= '<span class="button button-disabled">ر</span>';
            }
            ...
            if ($hasWomenFile) {
                ...
                // رابط قابل للنقر في PDF - استخدام href مباشر مع target="_blank"
                $html .= '<a href="' . htmlspecialchars($fullWomenUrl, ENT_QUOTES, 'UTF-8') . '" target="_blank" class="button button-women" style="text-decoration: none !important; color: #FFFFFF !important; display: inline-block;">ن</a>';
            } else {
                $html .= '<span class="button button-disabled">ن</span>';
            }
        ...
        // تفعيل الروابط في PDF
        $mpdf->SetHTMLFooter('');
        
        // كتابة HTML مع الروابط - mPDF يدعم الروابط تلقائياً من HTML
        // تأكد من أن الروابط تحتوي على URLs كاملة (http:// أو https://)
        $mpdf->WriteHTML($html);
        ...
        // إذا فشل mPDF، استخدم export_pdf.php
